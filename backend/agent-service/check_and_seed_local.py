...
# Updated to use root/Raje2003 as per .env
DATABASE_URL = "mysql+aiomysql://root:Raje2003@localhost:3306/boardroom"
...
async def check_and_seed():
    ...
    try:
        ...
        async with engine.begin() as conn:
            # Check agents
            result = await conn.execute(text("SELECT COUNT(*) FROM agents WHERE tenant_id='default'"))
            count = result.scalar()
